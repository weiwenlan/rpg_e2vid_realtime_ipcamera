...
def Fixedreturn(address , N):
    if address != None:
         with AedatFile(address) as f:
            events = np.hstack([packet for packet in f['events'].numpy()])
    else:
        with NetworkEventInput(address='127.0.0.1', port=7777) as f:
            events = np.hstack([packet for packet in f])
    # Access information of all events by type
    timestamps, x, y, polarities = events['timestamp'], events['x'], events['y'], events['polarity']
    #import pdb; pdb.set_trace()
    dataline=np.empty([1,4])
    for i in range(len(events)):
        timestamps, x, y, polarities = events['timestamp'][i], events['x'][i], events['y'][i], events['polarity'][i]
...
if __name__ == "__main__":
    ...
    event_window_iterator=Fixedreturn(args.row_file,N)
